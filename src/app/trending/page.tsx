      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex justify-center gap-4 mt-12">
            {page > 1 && (
              <Link href={`/trending?page=${page - 1}`} className="px-6 py-2 bg-zinc-800 text-white rounded-lg hover:bg-zinc-700">
                Previous
              </Link>
            )}
            {hasNext && (
              <Link href={`/trending?page=${page + 1}`} className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
                Next
              </Link>
            )}
          </div>
        ...
        <div className="text-center py-16">
          <h3 className="text-xl text-gray-400">No trending content found</h3>
        </div>
      )}
    </>
  );
  ...
  return (
    <div className="min-h-screen bg-black pt-32 px-4 md:px-8 pb-16">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl md:text-4xl font-bold text-white mb-8">🔥 Trending Now</h1>
        <Suspense fallback={
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {[...Array(12)].map((_, i) => (
              <div key={i} className="aspect-[2/3] skeleton rounded-lg" />
